...
class ExternalProgram(Program):
    ...
    def __init__(self, name: str, command: T.Optional[T.List[str]] = None,
        ...
        self.interpreter_args: T.List[str] = []
        self.script_args: T.List[str] = []
        ...
        if self.found():
            # Find the script path (last item that is a file) and split command into parts.
            # For ['python', '-u', 'script.py', '--arg'], we get:
            #   _interpreter = ['python', '-u']
            #   path = 'script.py'
            #   _script_args = ['--arg']
            path_idx = None
            ...
            if path_idx is not None:
                self.interpreter_args = self.command[:path_idx]
                self.path = self.command[path_idx]
                self.script_args = self.command[path_idx + 1:]
            else:
                # No file found - first element is the program itself
                self.interpreter_args = []
                self.path = self.command[0]
                self.script_args = self.command[1:]
    ...
    def get_interpreter(self) -> T.List[str]:
        """Return the interpreter and its arguments (before the script path)."""
        return self.interpreter_args[:]
